...
{
    internal sealed class ReachyAsrTranscriptionProtocolParser
    {
        private readonly string text;
        private readonly int maximumDepth;
        private readonly int maximumTranscriptCharacters;
        private int index;

        public ReachyAsrTranscriptionProtocolParser(
            string text,
            int maximumDepth,
            int maximumTranscriptCharacters)
        {
            ...
            if (maximumTranscriptCharacters < 1 ||
                maximumTranscriptCharacters > 1024 * 1024)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(maximumTranscriptCharacters));
            }
            this.maximumDepth = maximumDepth;
            this.maximumTranscriptCharacters = maximumTranscriptCharacters;
        }
        ...
        public string Parse()
        {
            ...
            if (!TryConsume('}'))
            {
                while (true)
                {
                    ...
                    if (property == "text")
                    {
                        RequireFirstOccurrence(ref sawText, "text");
                        transcript = ReadString();
                        if (transcript.Length > maximumTranscriptCharacters)
                        {
                            throw Error(
                                "ASR transcript exceeds the configured character bound.");
                        }
                    }
                    else
                    {
            ...
            if (!sawText || transcript == null)
            {
                throw Error("ASR transcription response omitted text.");
            }
            return transcript;
        }
            ...
            switch (escape)
            {
                ...
                case 'u':
                    ...
                    if (char.IsHighSurrogate(decoded))
                    {
                        ...
                        {
                            throw Error(
                                "JSON unicode escape contains an unpaired high surrogate.");
                        }
                        index += 2;
                        ...
                        {
                            throw Error(
                                "JSON unicode escape contains an invalid surrogate pair.");
                        }
                    ...
                    {
                        throw Error(
                            "JSON unicode escape contains an unpaired low surrogate.");
                    }
                    builder.Append(decoded);
                    return;
                default:
        ...
        private void EnsureDepth(int depth)
        {
            ...
            {
                throw Error("JSON nesting exceeds the ASR protocol depth bound.");
            }
        }

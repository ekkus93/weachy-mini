...
SOURCE_FILE="${ROOT_DIR}/native/llama_runtime/benchmark/rma133_benchmark_v6.c"
...
if (( ${#pins[@]} != 3 )); then
    printf '%s\n' 'Could not read the RMA-133 V6 Android toolchain pins.' >&2
    exit 1
fi
EXPECTED_NDK="${pins[0]}"
MIN_SDK="${pins[1]}"
...
if [[ ! -s "${RUNTIME_LIBRARY}" ]]; then
    printf 'RMA-133 V6 requires the ABI-2 Android runtime at %s.\n' "${RUNTIME_LIBRARY}" >&2
    exit 1
fi
if [[ ! -f "${SOURCE_FILE}" || ! -f "${HEADER_DIR}/reachy_llama.h" ]]; then
    printf '%s\n' 'RMA-133 V6 benchmark source/header is missing.' >&2
    exit 1
fi
...
if [[ ! -x "${CLANG}" || ! -x "${READELF}" || ! -x "${NM}" ]]; then
    printf '%s\n' 'Pinned NDK ARM64 compiler or inspection tools are missing.' >&2
    exit 1
fi
if ! "${NM}" -D --defined-only "${RUNTIME_LIBRARY}" \
    | awk '{print $NF}' \
    | grep -E '^reachy_llama_generation_start_constrained(@@REACHY_LLAMA_1)?$' >/dev/null; then
    printf '%s\n' 'ABI-2 constrained-generation export is missing from libreachy_llama.so.' >&2
    exit 1
fi
...
"${CLANG}" \
    ...
    -lm \
    -ldl \
    -o "${OUTPUT_DIR}/rma133_benchmark_v6"

"${READELF}" -d "${OUTPUT_DIR}/rma133_benchmark_v6" > \
    "${OUTPUT_DIR}/rma133_benchmark_v6.dynamic.txt"
if grep -E 'lib(c\+\+|stdc\+\+|llama|ggml)' "${OUTPUT_DIR}/rma133_benchmark_v6.dynamic.txt"; then
    printf '%s\n' 'RMA-133 V6 benchmark has a prohibited direct runtime dependency.' >&2
    exit 1
fi
if ! grep -F 'libreachy_llama.so' "${OUTPUT_DIR}/rma133_benchmark_v6.dynamic.txt" >/dev/null; then
    printf '%s\n' 'RMA-133 V6 benchmark is not linked through the first-party reachy_llama ABI.' >&2
    exit 1
fi
sha256sum "${OUTPUT_DIR}/rma133_benchmark_v6" > "${OUTPUT_DIR}/rma133_benchmark_v6.sha256"
printf 'RMA-133 V6 Android benchmark built: %s\n' "${OUTPUT_DIR}/rma133_benchmark_v6"

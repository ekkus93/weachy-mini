...
OLD_HEADER = """## RMA-110 — Define vision provider contracts
...
NEW_HEADER = """## RMA-110 — Define vision provider contracts
...
REPORT = """# RMA-110 Vision Provider Contracts Validation
...
def main() -> None:
    ...
    if todo.count(OLD_HEADER) != 1:
        raise SystemExit("unexpected RMA-110 TODO header")
    if todo.count(RMA111_MARKER) != 1:
        raise SystemExit("unexpected RMA-111 marker")
    todo = todo.replace(OLD_HEADER, NEW_HEADER)
    todo = todo.replace(RMA111_MARKER, COMPLETION_EVIDENCE + RMA111_MARKER)

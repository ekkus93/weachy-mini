...
def validate_config(path: Path, cases_path: Path | None = None) -> dict[str, Any]:
    ...
    if (
        cases_path is not None
        and cases_path.resolve() != (ROOT / contract["behavior_cases_path"]).resolve()
    ):
...
def _groups(raw: str, line: int) -> tuple[tuple[str, ...], ...]:
    ...
    groups = tuple(
        tuple(term.strip().casefold() for term in group.split("|") if term.strip())
...
def load_cases(path: Path) -> list[Case]:
    ...
    header = [
        ...
        "expected_gesture",
        "expected_urgency",
        "speech_required_groups",
        "speech_forbidden_terms",
    ]
    ...
    for line_no, line in enumerate(lines[1:], 2):
        ...
        forbidden_terms = (
            ()
            if forbidden == "-"
            else tuple(t.strip().casefold() for t in forbidden.split("|") if t.strip())
        )
        result.append(
            Case(
                case_id,
                None if gaze_kind == "-" else gaze_kind,
                None if gaze_entity == "-" else gaze_entity,
                expression,
                gesture,
                urgency,
                _groups(required, line_no),
                forbidden_terms,
            )
        )
    if len(result) != 12:
...
def score_case(case: Case, record: dict[str, Any]) -> dict[str, Any]:
    ...
    semantic: list[str] = []
    score = 10.0
    if all(any(term in speech for term in group) for group in case.speech_groups) and not any(
        term in speech for term in case.forbidden
    ):
        score += 25.0
    else:
    ...
    if case.gaze_kind is None:
        ...
        and gaze.get("kind") == case.gaze_kind
        and gaze.get("entity_id") == case.gaze_entity
    ):
        score += 25.0
    else:
        semantic.append("gaze target mismatch")
    for key, expected, points in (
        ("expression", case.expression, 15.0),
        ("gesture", case.gesture, 15.0),
        ("urgency", case.urgency, 10.0),
    ):
...
def score_candidate(
    config_path: Path, cases_path: Path, raw_path: Path, candidate_id: str, output_path: Path
) -> dict[str, Any]:
    ...
    case_scores = [
        score_case(c, by_case[c.case_id])
        if c.case_id in by_case
        else {
            "case_id": c.case_id,
            "schema_valid": False,
            "semantic_score": 0.0,
            "reasons": ["required case missing"],
        }
        for c in cases
    ]
    ...
    expected_constraint = {
        ...
        "constraint_type": "GBNF",
        "grammar_path": CONSTRAINT["grammar_path"],
        "grammar_sha256": CONSTRAINT["grammar_sha256"],
        "grammar_root": "root",
        "constrained_mode_active": True,
        "constrained_start_attempts": 12,
        "constrained_start_successes": 12,
        "terminal_error_status": 0,
        "base_exit_code": 0,
    }
    constraint_reasons = [
        f"constraint evidence mismatch for {k}"
        for k, v in expected_constraint.items()
        if constraint.get(k) != v
    ]
    ...
    rates = [
        ...
        if _finite(r.get("decode_tokens_per_second")) and float(r["decode_tokens_per_second"]) > 0
    ]
    ...
    peak = max(
        (
            ...
            for r in records
            if isinstance(r.get("peak_rss_bytes"), int) and r["peak_rss_bytes"] >= 0
        ),
        default=0,
    )
    ...
    temps = [
        float(r[k])
        for r in records
        for k in ("battery_temp_before_c", "battery_temp_c", "battery_temp_after_c")
        if _finite(r.get(k)) and float(r[k]) > 0
    ]
    battery_ok = (
        _finite(before)
        and float(before) > 0
    ...
    ttft = [
        float(r["time_to_first_text_ms"])
        for r in case_records
        if _finite(r.get("time_to_first_text_ms")) and float(r["time_to_first_text_ms"]) >= 0
    ]
    reject = list(constraint_reasons)
    checks = [
        (
            completed != policy["required_completed_cases"],
            f"completed {completed}/12 required cases",
        ),
        (
            schema < policy["minimum_schema_reliability"],
            f"schema reliability {schema:.3f} below gate",
        ),
        (
            quality < policy["minimum_semantic_quality_score"],
            f"semantic quality {quality:.2f} below gate",
        ),
        (
            decode < policy["minimum_mean_decode_tokens_per_second"],
            f"decode {decode:.2f} tok/s below gate",
        ),
        (peak <= 0 or peak > policy["maximum_peak_rss_bytes"], f"peak RSS {peak} outside gate"),
        (not battery_ok, "battery temperature unavailable"),
        (
            battery_ok and battery_peak >= policy["maximum_battery_temperature_c"],
            f"battery peak {battery_peak:.1f} C reached gate",
        ),
        (
            battery_ok and battery_rise > policy["maximum_battery_temperature_rise_c"],
            f"battery rise {battery_rise:.1f} C above gate",
        ),
    ]
    ...
    report = {
        ...
        "constraint_evidence_valid": not constraint_reasons,
        "measurements": {
            "load_time_ms": float(model.get("load_time_ms", math.inf)),
            "parameter_count": model.get("parameter_count"),
            "tensor_bytes": model.get("tensor_bytes"),
            "training_context_tokens": model.get("training_context_tokens"),
            "peak_rss_bytes": peak,
            "schema_reliability": schema,
            "semantic_quality_score": quality,
            "mean_decode_tokens_per_second": decode,
            "mean_time_to_first_text_ms": statistics.fmean(ttft) if ttft else None,
            "battery_temp_before_c": before,
            "battery_temp_after_c": after,
...
def select_candidate(
    ...
    eligible.sort(
        key=lambda r: (
            -float(r["measurements"]["semantic_quality_score"]),
            -float(r["measurements"]["schema_reliability"]),
            -float(r["measurements"]["mean_decode_tokens_per_second"]),
            int(r["measurements"]["peak_rss_bytes"]),
            float(r["measurements"]["load_time_ms"]),
            str(r["candidate_id"]),
        )
    )
    ...
    print(
        json.dumps({"status": result["status"], "selected_candidate_id": selected}, sort_keys=True)
    )
    return result
...
def main() -> int:
    ...
    try:
        if args.command == "validate":
            validate_config(args.config, args.cases)
            load_cases(args.cases or ROOT / CONSTRAINT["behavior_cases_path"])
            return 0
        if args.command == "score":
            report = score_candidate(
                args.config, args.cases, args.raw, args.candidate_id, args.output
            )
            print(
                json.dumps(
                    {"candidate_id": report["candidate_id"], "eligible": report["eligible"]},
                    sort_keys=True,
                )
            )
            return 0
        result = select_candidate(args.config, args.report, args.output)
        return 0 if result["selected_candidate_id"] else 1

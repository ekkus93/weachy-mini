...
class Rma163ImportedContentSecurityTests(unittest.TestCase):
    def test_imported_json_reads_are_bounded_and_strict_utf8(self) -> None:
        ...
        for token in ("MaximumCameraCalibrationBytes", "MaximumLocalModelManifestBytes", "MaximumLocalVlmManifestBytes", "MaximumDurableSettingsBytes", "MaximumProviderProfilesBytes", "MaximumFallbackPoliciesBytes", "MaximumLocalModelMetadataBytes", "throwOnInvalidBytes: true", "ReadBoundedUtf8File", "RequireBoundedUtf8Text"):
            self.assertIn(token, policy)
        expected = {"ReachyCameraCalibrationPersistence.cs": "CameraCalibration", "ReachySettingsPersistence.cs": "DurableSettings", "ReachyProviderProfilePersistence.cs": "ProviderProfiles", "ReachyFallbackPolicyPersistence.cs": "FallbackPolicies"}
        ...
        readiness = (CORE / "LocalModels/ReachyLocalModelPackageManager.Readiness.cs").read_text(encoding="utf-8")
        package_contracts = (CORE / "LocalModels/ReachyLocalModelPackageContracts.cs").read_text(encoding="utf-8")
    ...
    def test_calibration_schema_and_numeric_ranges_are_bounded(self) -> None:
        ...
        profile = (CORE / "Application/ReachyCameraCalibrationProfile.cs").read_text(encoding="utf-8")
        intrinsics = (CORE / "Application/ReachyCameraIntrinsics.cs").read_text(encoding="utf-8")
        provider_persistence = (APP / "ReachyProviderProfilePersistence.cs").read_text(encoding="utf-8")
        state = (CORE / "Application/ReachyCameraCalibrationStateStore.cs").read_text(encoding="utf-8")
        self.assertIn("MaximumProfiles = ReachyCameraCalibrationStateStore.MaximumProfiles", persistence)
    ...
    def test_model_download_and_provider_urls_use_central_host_policy(self) -> None:
        ...
        download = (CORE / "LocalModels/ReachyLocalModelPackageManager.Download.cs").read_text(encoding="utf-8")
        transport = (CORE / "LocalModels/HttpLocalModelDownloadTransport.cs").read_text(encoding="utf-8")
        vlm = (CORE / "Perception/ReachyLocalVlmManifestContracts.cs").read_text(encoding="utf-8")
        for token in ("RequirePublicHttpsUri", "IsTrustedLocalDevelopmentHost", 'host.EndsWith(".local"', "IPAddress.IsLoopback", "bytes[0] == 169 && bytes[1] == 254", "bytes[0] == 10", "bytes[0] == 192 && bytes[1] == 168"):
    ...
    def test_path_traversal_and_arbitrary_model_overwrite_stay_denied(self) -> None:
        manifest = (CORE / "LocalModels/ReachyLocalModelManifest.cs").read_text(encoding="utf-8")
        paths = (CORE / "LocalModels/ReachyLocalModelPackageManager.Paths.cs").read_text(encoding="utf-8")
    ...
    def test_diagnostic_bundle_gate_denies_secret_and_private_media(self) -> None:
        security = (CORE / "Security/ReachyImportedContentSecurity.cs").read_text(encoding="utf-8")
        for token in ("IncludeSecretsByDefault = false", "IncludePrivateMediaByDefault = false", "Secret = 1", "PrivateMedia = 2", "kind == ReachyDiagnosticArtifactKind.RedactedText"):
            self.assertIn(token, security)
